        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#ffffff]">
              <th className="text-left px-6 py-2.5 font-mono text-[9px] tracking-[1.5px] uppercase text-[#888888]">Página</th>
          ...
          <tbody className="divide-y divide-[#e8e8e8]">
            {pages.map(p => (
              <tr key={p.page_path} className="hover:bg-[#ffffff] transition-colors">
                <td className="px-6 py-2.5 font-mono text-xs text-[#1a4fa0]">{p.page_path}</td>
                <td className="px-6 py-2.5 font-mono text-xs text-right text-[#000000] font-medium">{p.sessions.toLocaleString()}</td>
                <td className="px-6 py-2.5 font-mono text-xs text-right text-[#555555]">{p.users.toLocaleString()}</td>
                <td className="px-6 py-2.5 font-mono text-xs text-right text-[#000000]">{p.conversions}</td>
                <td className="px-6 py-2.5 text-right">
                  <span className={`font-mono text-xs ${p.conversion_rate > 3 ? 'text-[#1a7a4a]' : 'text-[#888888]'}`}>
    ...
    <div className="p-8">
      <div className="bg-[#fff0f2] border border-[#F7415C] border-l-4 border-l-[#F7415C] px-5 py-4">
        ...
        {message.includes('Configuración') && (
          <a href="/config" className="inline-block mt-3 font-mono text-[10px] uppercase tracking-wide bg-[#000000] text-white px-4 py-2 hover:bg-[#F7415C] transition-colors">
            Ir a Configuración →
          </a>
        )}
      </div>
